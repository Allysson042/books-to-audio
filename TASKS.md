...
- 2026-09-23 (macros 12-14, fim da pendência S1): `ensure_macros()` agora usa `build_chain()` com fallback (era só Gemini). Bugfix `OpenRouterProvider`: router grátis sorteou modelo de reasoning (`content: null`) — usa `reasoning` ou falha p/ fallback. `.env` fixado em `OPENROUTER_MODEL=qwen/qwen3.8-27b:free` (ctx 262k; lista free varia, reconsultar `/api/v1/models`). **14/14 macros via LLM** (12 via Gemini, 13-14 via Qwen — com leve ruído de formato, só contexto interno). Bugfix `infos` não inicializado. BM25 restaurado p/ 196 docs após ingests parciais.
